function dp = post_dpf_corrections_frommat(dp, filename_prefix)

%{
%addpath(genpath(['/global/homes/p/pterman/LUXcode/Stable_Releases/v2.0/DataProcessing']));
filename_prefix = filename_prefix(1:19)
pathbase= ;
data_path_evt = [pathbase filesep filename_prefix filesep];
filename_rq = strrep(filename_evt,'evt','rq');
%new_filename_rq = [filename_rq(1:end-3) '_new.rq']
data_path_rq = [pathbase filesep];
%}
%filename_evt='luxsm_20130624T0010_f000000001.evt'%
%filename_prefix = filename_evt(1:19)
filename_prefix = char(filename_prefix);
...
%dp = load([pathbase filesep 'matfiles_old' filesep filename_rq '.mat']);
...
fprintf('Done with correcting the energies and positions!\n')
end
%status = LUXBinaryRQWriter_framework(settings, dp, new_filename_rq, data_path_rq, dp.event_number, livetime);

%status =AdditionalFileFormat_SaveMatFile(filename_evt,data_path_evt, new_filename_rq,data_path_rq,data_processing_xml_path,iq_xml_path)
%this is to save the .rq file as a .mat file saved 

%system(['/global/u2/p/pterman/LUXcode/Stable_Releases/v2.0/DataProcessing/CppModules/bin/rq_to_rqroot ' filename_evt ' ' data_path_evt ' ' new_filename_rq ' ' pathbase ' 1 ' data_processing_settings_path ' ' lug_iqs_xml_file ])
%this is to save as a .root file as well
